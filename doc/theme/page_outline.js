/* SPDK Doxygen enhancements — loaded via HTML_EXTRA_FILES after navtree.js.
   ...
   2. Fix right-panel TOC scroll and add collapse/expand for the page outline
      (navtree.js targets #doc-content which no longer scrolls; we use window
      scroll + scrollIntoView, and add expand/collapse with visibility tracking)
   3. Toggle buttons for left and right panels (outside panel boundaries)
   4. Fix right-panel resize drag (Doxygen's formula assumes container
...
/* ----------------------------------------------------------------
   2. Fix right-panel TOC scroll and add collapse/expand behaviour.
      Scroll via window (not #doc-content), and add expand/collapse
      with visibility tracking that auto-expands sections currently
      in view.
   ---------------------------------------------------------------- */
setTimeout(function(){
